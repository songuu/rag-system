...
生产读路径或可注入 seam。真实 Milvus hybrid 切流、Graph artifact producer、PDF visual
caller 与外部 durable provider 仍属于 live activation，不由代码闭环冒充上线完成。
...
- **E3**：hybrid 实现 injectable capability/search port、独立 sparse+dense candidate pool、RRF/
  ...
  零 provider 工作，shadow/active 有 call/input/output/concurrency 上限和版本化 identity。
...
- **E5**：新增 exact scope/document/version/trust 的内存/文件 artifact store、Graph entity/community
  ...
  对 node/edge/reference/operation 有上限并按批次 yield，使 timer-driven abort 可抢占。
...
- **E7**：checkpoint v2 绑定 workflow/version、scope、document version、job fingerprint 和
  ...
  retry/takeover 复用稳定 step execution ID，语义明确为 at-least-once。
- **T17**：新增 21-case hermetic production-policy control-plane gate，并将 security、Kernel、
  E1b、matrix、contract、typecheck 与 full server build 串入 CI validate job。

### Verification

- `pnpm test`：49 个测试文件、492/492 通过；默认链同时执行 E1b、matrix 与 production contract。
- `pnpm rag:eval:e1b`：8/8；`pnpm rag:eval:matrix`：两个 target 各 8/8；均为 0 security violation。
- `pnpm rag:eval:contracts`：21/21，`executionMode=hermetic-in-process`，明确
  `productionQualityMeasured=false`。
- TypeScript `--noEmit --incremental false`、触及文件 ESLint、`git diff --check` 通过。
- Next.js 16.2.6 production build 通过，88/88 页面生成；`/api/ask` 与 `/api/mirofish/graph`
  保持动态服务路由。
...
- production hybrid 仍需真实 Milvus 2.6 capability probe、shadow collection、回填、p95/质量门禁
  与回滚演练；当前 E3 是可注入 seam，不是已切流 collection。
- E5 production read path 依赖精确预置 File artifact；builder 尚未写入该 store，文件 TTL/delete/
  多实例共享 quota 需由外部 lifecycle/provider 接线。
- E6 尚无 ingest/index/query production caller，也未启用真实视觉模型；E7 默认仍是
  `processPersistent: false` 的 in-memory store，未证明跨进程 restart 或 HITL。
- 以上 live activation 未完成项不影响当前 dense/text fallback，也不降低已完成代码合同的状态。
