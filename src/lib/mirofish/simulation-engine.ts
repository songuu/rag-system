...
如果存在行为锚点, 你必须让行为符合 posting_style、stance、opinion_drift_rate 和 influence_weight。
不要让所有 Agent 表现得像同一种人；观察者少发声、扩音者更容易转发、意见领袖更坚定且更有影响。
...
export class SimulationEngine {
  ...
  async executeRound(
    ...
    // 模拟互动（点赞/回复计数）
    this.simulateInteractions(roundPosts);
  ...
  /** 选择活跃 Agent */
  private selectActiveAgents(profiles: EntityProfile[], count: number, now: Date): EntityProfile[] {
    if (!profiles.some(profile => profile.behavioral_anchors)) {
    ...
    const shuffled = [...profiles].sort(() => Math.random() - 0.5);
    return shuffled
      .sort((a, b) => this.getActivityScore(b, now) - this.getActivityScore(a, now))
      .slice(0, Math.min(count, profiles.length));
  }

  /** 根据行为锚点调节活跃 Agent 抽样；无锚点时保持旧的随机选择。 */
  private getActivityScore(profile: EntityProfile, now: Date): number {
    const anchors = profile.behavioral_anchors;
    if (!anchors) return Math.random();

    const currentHour = now.getHours();
    const hourBoost = anchors.active_hours.includes(currentHour) ? 1.35 : 0.85;
    const stanceBoost = anchors.stance === 'amplifier'
      ? 1.25
      : anchors.stance === 'observer'
        ? 0.7
        : 1;
    return Math.random() * anchors.influence_weight * hourBoost * stanceBoost;
  }

  /** 模拟互动（仅更新 newPosts 内部的计数，不修改 existingPosts） */
  private simulateInteractions(newPosts: SimulationPost[]): void {
