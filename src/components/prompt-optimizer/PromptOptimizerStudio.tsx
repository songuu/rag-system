...
import { ArrowLeft, Braces, Check, ChevronRight, Clock3, Copy, Eye, EyeOff, FileClock, KeyRound, LoaderCircle, Plus, Save, Settings2, Sparkles, X } from "lucide-react";
...
type Version = {
  ...
  prompt: string;
  instruction: string;
  analysis: { summary?: string; improvements?: string[] };
...
export default function PromptOptimizerStudio() {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        ...
        setProfileDraft((current) => ({ ...current, token: "" }));
      }
    };
  ...
  async function refreshCatalogs() {
    try {
      const [profileData, templateData, workspaceData] = await Promise.all([api<Profile[]>("/models"), api<Template[]>("/templates"), api<Workspace[]>("/workspaces")]);
    ...
    try {
      ...
      const data = await api<{ workspaceId: string; version: Version }>("/optimize", {
        ...
        body: JSON.stringify({
          ...
          templateId: template?.id ?? `${mode}-v1`,
  ...
  async function openWorkspace(workspaceId: string) {
    ...
    try {
      ...
      setPrompt(data.workspace.original_prompt);
      const latest = data.versions[0] ?? null;
      setVariables(latest?.variables || data.workspace.variables || {});
      ...
      if (requestSequence === workspaceRequestSequence.current) setError(messageOf(caught));
    } finally {
      if (requestSequence === workspaceRequestSequence.current) setBusy(false);
    }
  }
  ...
  async function saveManual() {
    ...
    setError("");
    try {
      await api(`/workspaces/${encodeURIComponent(workspace.workspace_id)}/versions`, {
        method: "POST",
        body: JSON.stringify({
          ...
          variables,
          modelProfileId: selectedProfileId,
          templateId: activeVersion.templateId || template?.id || `${mode}-v1`,
          parentVersion: activeVersion.versionNumber,
      ...
      const saved = await api<Profile>("/models", {
        ...
        body: JSON.stringify({
          ...
          settings: {
            ...
            topP: profileDraft.topP,
            maxTokens: profileDraft.maxTokens,
            timeoutMs: profileDraft.timeoutSeconds * 1000,
          },
        }),
      });
      await refreshCatalogs();
  ...
  function selectVersion(version: Version) {
    ...
    setMode(modeForVersion(version, mode));
    if (version.modelProfileId) setSelectedProfileId(version.modelProfileId);
  }
  async function copyResult() {
    if (!activeVersion) return;
    try {
      await navigator.clipboard.writeText(activeVersion.prompt);
      setCopied(true);
    ...
    <div className={styles.shell}>
      <header className={styles.header}>
        <div className={styles.brand}>
          <Link href="/" className={styles.back} aria-label="返回首页">
            <ArrowLeft size={18} />
          </Link>
          <div className={styles.mark}>P°</div>
          <div>
            <strong>提示词优化台</strong>
            <span>Prompt Atelier / v1</span>
          </div>
        </div>
        <div className={styles.headerActions}>
          ...
          <button className={styles.primarySmall} disabled={busy} onClick={startFresh}>
            <Plus size={17} />
            新建
          </button>
        </div>
      ...
      <section className={styles.intro}>
        <div>
          <span className={styles.eyebrow}>MAKE THE ASK SHARPER</span>
          <h1>
            把模糊想法，打磨成
            <br />
            <em>可执行的提示词。</em>
        ...
        <p>
          独立模型配置 · PostgreSQL 版本留痕 · 变量化复用
          <br />
          每次优化都成为一个可回看的版本，而不是覆盖。
        </p>
      </section>
      ...
      <div className={styles.workspace}>
        <aside className={styles.history}>
          ...
          <div className={styles.historyList}>
            {workspaces.length === 0 ? (
              ...
              workspaces.map((item) => (
                <button disabled={busy} key={item.workspace_id} className={workspace?.workspace_id === item.workspace_id ? styles.historyActive : styles.historyItem} onClick={() => void openWorkspace(item.workspace_id)}>
                  <span>{item.title}</span>
                  <small>
                    v{item.current_version} · {MODE_LABELS[item.mode]}
                  </small>
        ...
        <section className={styles.editor}>
          <div className={styles.editorHead}>
            <div>
              ...
              <div>
                <h2>原始提示词</h2>
                <p>
                  保留变量请使用 <code>{"{{variable}}"}</code>
                </p>
              </div>
            </div>
            <span className={styles.counter}>{prompt.length.toLocaleString()} / 20,000</span>
          </div>
          <div className={styles.modes}>
            {(["general", "structured", "image"] as Mode[]).map((item) => (
              <button key={item} className={mode === item ? styles.modeActive : styles.mode} onClick={() => setMode(item)}>
                {MODE_LABELS[item]}
              </button>
            ))}
          </div>
          ...
          <label className={styles.instruction}>
            ...
            <input value={instruction} maxLength={2000} onChange={(event) => setInstruction(event.target.value)} placeholder="如：保持中文、增加验收标准、减少营销腔" />
          </label>
          {variableNames.length > 0 && (
            <div className={styles.variables}>
              <div className={styles.variableTitle}>
                <Braces size={16} />
                变量试填 <span>{variableNames.length}</span>
              </div>
          ...
          <div className={styles.optimizeBar}>
            <div>
              <Sparkles size={18} />
              <span>{template?.name ?? MODE_LABELS[mode]}</span>
              <small>{template?.description}</small>
            </div>
            <button disabled={busy} onClick={() => void optimize(Boolean(workspace))}>
              {busy ? <LoaderCircle className={styles.spin} size={18} /> : <Sparkles size={18} />} {workspace ? "基于当前版本迭代" : "开始优化"}
            </button>
          </div>
          {error && (
            <div className={modelOpen ? styles.modalError : styles.error} role="alert">
              {error}
        ...
        <section className={styles.result}>
          <div className={styles.resultHead}>
            <div>
              <span className={styles.stepDark}>02</span>
              <div>
                <h2>优化版本</h2>
                <p>{activeVersion ? `已选 v${activeVersion.versionNumber}` : "等待第一次优化"}</p>
              </div>
            </div>
            {activeVersion && (
              <div className={styles.resultActions}>
                ...
                <button disabled={busy} onClick={() => void saveManual()}>
                  <Save size={15} />
                  存为新版本
                </button>
              </div>
          ...
          {!activeVersion ? (
            <div className={styles.emptyResult}>
              <div>✦</div>
              <h3>好提示词不是一次写成的。</h3>
              <p>左侧输入目标、选择模式与模型，优化结果会在这里以不可变版本保存。</p>
            </div>
          ) : (
            <>
              <div className={styles.paper}>
                <div className={styles.paperMeta}>
                  <span>VERSION {String(activeVersion.versionNumber).padStart(2, "0")}</span>
                  <span>{activeVersion.kind.toUpperCase()}</span>
                </div>
                <textarea value={activeVersion.prompt} onChange={(event) => setActiveVersion((current) => (current ? { ...current, prompt: event.target.value } : current))} />
                <div className={styles.analysis}>
                  <strong>为什么这样改</strong>
                  <p>{activeVersion.analysis?.summary || "该模型返回了优化正文，可继续迭代或手动调整后存为新版本。"}</p>
                  {activeVersion.analysis?.improvements?.map((item) => (
          ...
          {versions.length > 0 && (
            <div className={styles.timeline}>
              ...
              <div>
                {versions.map((item) => (
                  <button disabled={busy} key={item.versionNumber} className={activeVersion?.versionNumber === item.versionNumber ? styles.versionActive : styles.version} onClick={() => selectVersion(item)}>
                    <span>v{item.versionNumber}</span>
                    <small>
                      {item.kind} · {formatTime(item.createdAt)}
                    </small>
                  </button>
                ))}
              </div>
      ...
      {modelOpen && (
        ...
        >
          <div className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="prompt-model-title" onMouseDown={(event) => event.stopPropagation()}>
            <button
            ...
            <div className={styles.formGrid}>
              ...
              <label>
                ...
                >
                  ...
                  {process.env.NODE_ENV !== "production" && <option value="ollama">Ollama（仅本地）</option>}
                </select>
              </label>
              <label>
                模型名称
              ...
              {["compatible", "ollama"].includes(profileDraft.provider) && (
                <label>
                  Base URL
                  <input
                ...
                <label className={styles.credentialField}>
                  ...
                  <span className={styles.secretInput}>
                    <input
                      ...
                      placeholder="sk-••••••••••••••••"
                    />
                    <button type="button" onClick={() => setTokenVisible((visible) => !visible)} aria-label={tokenVisible ? "隐藏 Token" : "显示 Token"}>
            ...
            <button className={styles.modalSubmit} disabled={busy} onClick={() => void saveProfile()}>
              {busy ? <LoaderCircle className={styles.spin} /> : <Settings2 />}
              保存模型档案
            </button>
          </div>
        </div>
...
function modeForVersion(version: Version | null, fallback: Mode): Mode {
  const candidate = version?.templateId.split("-")[0];
  return candidate === "general" || candidate === "structured" || candidate === "image" ? candidate : fallback;
}
