...
- [2026-09-08] [neo4j/postgres/outbox] PostgreSQL 发布指针到 Neo4j 检索状态的 Outbox 投影 — 知识图谱发布 API 已成功把 PostgreSQL Active Pointer 推进到新 revision，但 Ask 图检索持续返回 no gain。管理查询可以看到实体、社区和路径，因此问题很容易被误判为检索算法或模型质量问题。 → `docs/solutions/2026-09-08-neo4j-publication-outbox-projection.md`
